...
- `ecs.sh` 与独立的 `ipcheck.sh` 仅在 curl 明确报告主机名无法解析时，才会使用已验证的加密 DNS 引导重试
- 超时、TLS/HTTP/CDN/代理失败和丢包维持原有错误路径，不会误判为 DNS 缺失，也不会改写系统解析配置
- 回退端点按真实 DNS 查询结果和延迟选择，仅缓存于本次运行的临时目录；嵌入目录由 CI 同步，并覆盖 DNS 失败、缓存复用与超时边界测试
